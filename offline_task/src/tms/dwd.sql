...
select id,
       order_id,
       org_id,
       substr(inbound_time,1,10) inbound_time,
       inbound_emp_id
from ods_order_org_bound
where dt = '2025-07-13';
...
from (
    ...
    where dt = '2025-07-12'
      and is_deleted = '0'
         ...
         left join (select id, name from ods_base_dic where dt = '2025-07-12' and is_deleted = '0') dic_for_collect_type
                   on info.collect_type = cast(dic_for_collect_type.id as string);  -- 修正为dic_for_collect_type
