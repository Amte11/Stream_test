...
insert overwrite table dim_activity_full partition(`dt`='2025-07-03')
select
    rule.id,
    info.id,
    activity_name,
...
insert overwrite table dim_coupon_full partition(`dt`='2025-07-03')
select
    id,
    coupon_name,
    coupon_type,
...
insert overwrite table dim_promotion_pos_full partition(dt='2025-07-03')
select
    `id`,
    `pos_location`,
    `pos_type`,
...
insert overwrite table dim_promotion_refer_full partition(dt='2025-07-03')
select
    `id`,
    `refer_name`,
    `create_time`,
...
insert overwrite table dim_province_full partition(dt='2025-07-03')
select
    province.id,
    province.name,
    province.area_code,
...
insert overwrite table dim_sku_full partition(dt='2025-07-04')
select
    sku.id,
    sku.price,
    sku.sku_name,
...
load data inpath '/bigdata_warehouse/gmall/dim/date_info.txt' overwrite into table tmp_dim_date_info;
