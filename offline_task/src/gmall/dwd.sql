...
insert overwrite table dwd_interaction_favor_add_inc partition(dt='2025-07-03')
select
    data.id,
    data.user_id,
    data.sku_id,
...
--② 每日装载
insert overwrite table dwd_tool_coupon_used_inc partition(dt='2025-07-04')
select
    data.id,
    data.coupon_id,
    data.user_id,
...
insert overwrite table dwd_trade_cart_full partition(dt='2025-07-03')
select
    id,
    user_id,
    sku_id,
...
--② 每日装载
insert overwrite table dwd_trade_order_detail_inc partition (dt='2025-07-04')
select
    od.id,
    order_id,
    user_id,
...
insert overwrite table dwd_trade_pay_detail_suc_inc partition (dt='2025-07-04')
select
    od.id,
    od.order_id,
    user_id,
...
from
    (
        select
            ...
            data.split_coupon_amount
        from ods_order_detail data
        where (dt = '2025-07-04' or dt = date_add('2025-07-04',-1))
    ) od
        join
    (
        select
...
select
    ...
    nvl(nvl(oi.finish_time,log.finish_time),'9999-12-31') as dt  -- 添加别名
from
    (
        -- 历史数据部分（保持不变）
        select
...
insert overwrite table dwd_traffic_page_view_inc partition (dt='2025-07-03')
select
    common.province_id,
    common.brand,
    common.channel,
...
PARTITIONED BY (`dt` STRING)  -- 分区字段 dt
STORED AS parquet-- 显式指定存储格式
LOCATION '/bigdata_warehouse/gmall/dwd/dwd_user_login_inc/';

-- 插入数据（修正 log 列为 log，优化子查询）
insert overwrite table dwd_user_login_inc partition (dt = '2025-07-03')
select
    user_id,
    date_format(from_utc_timestamp(cast(ts as bigint)/1000, 'GMT+8'), 'yyyy-MM-dd') as date_id,  -- 日期ID
    date_format(from_utc_timestamp(cast(ts as bigint)/1000, 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as login_time,  -- 登录时间
    channel,
    province_id,
    version_code,
    mid_id,
...
from
    (
        ...
        where dt = '2025-07-03'  -- 源表分区格式假设为'yyyy-MM-dd'
...
-- 每日装载（静态分区）
insert overwrite table dwd_user_register_inc partition(dt='2025-07-03')
select
    ui.user_id,
    date_format(ui.create_time, 'yyyy-MM-dd') as date_id,
    ui.create_time,
