...
union
select '2025-07-03' dt,
       recent_days,
       sum(if(login_date_first >= date_add('2025-07-03', -recent_days + 1), 1, 0)) new_user_count,
...
where dt='2025-07-03'
  and order_date_first>=date_add('2025-07-03',-recent_days+1)
group by recent_days;
...
from
    (
        ...
        where dt>=date_add('2025-07-03',-6)
    )t1
where diff=2;
select
    count(distinct(user_id))
        ...
        from
            (
                select
                    user_id,
                    case dt
                        when '2025-07-03' then 1
                        when '2025-07-02' then 10
                        when '2025-07-01' then 100
                        when '2025-06-30' then 1000
                        when '2025-06-29' then 10000
                        when '2025-06-28' then 100000
                        when '2025-06-27' then 1000000
                        else 0
                        end num
                from dws_trade_user_order_1d
                where dt>=date_add('2025-07-03',-6)
...
(
    ...
    `order_to_pay_interval_avg` STRING COMMENT '下单到支付时间间隔平均值,单位为秒',
     `dt`                        STRING COMMENT '统计日期'
...
where dt in ('9999-12-31', '2025-07-03')
  and payment_date_id = '2025-07-03';
...
where dt>=date_add('2025-07-03',-recent_days+1)
...
SELECT
    COALESCE(churn.dt, back.dt) AS dt,
    COALESCE(churn.user_churn_count, 0) AS user_churn_count,
    COALESCE(back.user_back_count, 0) AS user_back_count
FROM (
         -- 原 churn 子查询
         select
             '2025-07-03' dt,
         ...
         where dt='2025-07-03'
           and login_date_last=date_add('2025-07-04',-7)
     ) churn
         FULL JOIN (
    -- 原 back 子查询
    select
    ...
    from
        (
            ...
            from dws_user_user_login_td
            where dt='2025-07-03'
              and login_date_last = '2025-07-04'
        )t1
            join
        (
            select
            ...
            from dws_user_user_login_td
            where dt=date_add('2025-07-03',-1)
        )t2
